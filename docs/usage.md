...
Firstly, it requires a .yaml file (described below in Full Samplesheet). This will contain all of the information required for running the TreeVal pipeline.

The cram files must each be accompanied by an index file (.crai) generated by samtools index.
...
for organism in geneset.split(',')
  path = {gene_alignment_dir}{classT}/{organism}/csv-data/{organism}-data.csv
```

Each csv file acts as a file of file names. Containing data per file in a format such as:

```bash
{organism},{cdna|pep|cds|rna},/lustre/scratch123/tol/resources/treeval/gene_alignment_data/{classT}/{organism}/{cds|cdna|rna|pep}/{organism}_{cdna|pep|cds|rna}.fasta
```

It is advised that the fasta files be no larger than 50Mb, this allows the pipeline to run without wasting significant resources on large alignments.
