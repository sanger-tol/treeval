...
- `assembly`
  ...
  - `classT`: Clade name (as used to group synteny sequences).
  - `asmVersion`:
  - `dbVersion`:
  - `gevalType`:
- `reference_file`: Sample .fa file
- `assem_reads`
  - `pacbio`: # Will be required in future development.
  - `hic`: # Will be required in future development.
  - `supplementary`: #Will be required in future development.
- `alignment` #Path should end up looking like "{data_dir}{classT}/{common_name}/csv_data/{geneset}-data.csv"
  - `data_dir`: Gene alignment data path
  - `common_name`: # For future implementation (adding bee, wasp, ant etc)
  - `geneset`: Geneset files
- `self_comp`
  - `motif_len`:
...
```console
nextflow run nf-core/treeval --input assets/treeval_test.yaml --outdir <OUTDIR> -profile singularity, sanger
```
