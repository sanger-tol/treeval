...
full_table = pd.read_csv(fulltable, sep="\t", comment="#")
...
df_new = (
    df_a.join(full_table[["buscoID"]])
    .join(full_table[["Score"]])
    .join(full_table[["Strand"]])
    .join(full_table[["OrthoDB url"]])
)
...
df_final.to_csv(genefile, index=False, header=False, sep="\t")
